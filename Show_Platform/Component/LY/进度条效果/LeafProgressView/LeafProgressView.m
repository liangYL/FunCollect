...
@property (nonatomic, retain) UIImageView *flowerView;
@property (nonatomic, retain) UIImageView *progressImageView;
@property (nonatomic, retain) UIImageView *progressBGImageView;
...
- (instancetype)initWithFrame:(CGRect)frame{
    if (self = [super initWithFrame:frame]) {
        ...
        _flowerView.backgroundColor = [UIColor orangeColor];
        [self addSubview:_flowerView];
        CAShapeLayer *layer = [CAShapeLayer layer];
        UIBezierPath *aPath = [UIBezierPath bezierPathWithOvalInRect:_flowerView.bounds];
        layer.path = aPath.CGPath;
        _flowerView.layer.mask = layer;
        ...
        self.textLabel = [[UILabel alloc] initWithFrame:CGRectMake(_progressBGImageView.center.x, 2, 30, 30)];
        _textLabel.text = @"100\%";
...
- (void)stopLoading{
    ...
    self.progressImageView.frame = _progressBGImageView.frame;
...
- (void)setProgress:(CGFloat)rate{
    ...
    leafLayer.contents = (__bridge id )([UIImage imageNamed:@"leaf"].CGImage);
