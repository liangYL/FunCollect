...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        [self.textLabel setMargin:UIEdgeInsetsMake(0, 0, 0, 0)];
...
- (CAShapeLayer *)createCircleShapeWithPosition:(CGPoint)position pathRect:(CGRect)rect radius:(CGFloat)radius
{
    ...
    if (_flashWidth) {
         circleShape.lineWidth = _flashWidth;//宽
    }else{
        circleShape.lineWidth = 2.5;//宽
    }
    return circleShape;
}
...
-(void)setFlashWidth:(CGFloat)borderWidth{
    _flashWidth = borderWidth;
}
