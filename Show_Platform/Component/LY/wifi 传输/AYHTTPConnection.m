...
#warning 此处修改了加载的文件夹
        NSString * str =[[paths objectAtIndex:0] stringByAppendingString:@"/lyResource"];
        
        NSDirectoryEnumerator *direnum = [[NSFileManager defaultManager] enumeratorAtPath:str];
...
#warning  修改缓存目录
    //缓存目录
    NSString *cachePath = [NSHomeDirectory() stringByAppendingPathComponent:@"Documents/lyResource"];
    ...
    NSString *uploadFilePath = [cachePath stringByAppendingPathComponent:fileName];
...
- (void) processContent:(NSData *)data WithHeader:(MultipartMessageHeader *)header
{
    if (storeFile)
    {
        ...
        NSDictionary *value = [NSDictionary dictionaryWithObjectsAndKeys:[NSNumber numberWithFloat:progress], @"progressvalue",[NSNumber numberWithInt:(int)data.length], @"cureentvaluelength", nil];
