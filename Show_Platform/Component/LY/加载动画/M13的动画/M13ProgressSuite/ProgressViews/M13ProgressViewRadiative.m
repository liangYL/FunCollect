...
@property(nonatomic,retain)NSMutableArray *ripplePaths;
...
- (void)setRippleWidth:(CGFloat)rippleWidth
{
    ...
    for (UIBezierPath *path in _ripplePaths) {
...
- (void)layoutSubviews
{
    ...
    for (int i = 0; i < _numberOfRipples - 1; i++) {
        if (_shape == M13ProgressViewRadiativeShapeCircle) {
            ...
            //Set the width
            path.lineWidth = _rippleWidth;
            [self.ripplePaths addObject:path];
...
- (void)drawRect:(CGRect)rect
{
    ...
    if (_progressOutwards) {
        enumerator = [_ripplePaths objectEnumerator];
    } else {
        enumerator = [_ripplePaths reverseObjectEnumerator];
    }
...
-(NSMutableArray *)ripplePaths{
    if (!_ripplePaths) {
        self.ripplePaths = [NSMutableArray array];
    }
    return _ripplePaths;
}
