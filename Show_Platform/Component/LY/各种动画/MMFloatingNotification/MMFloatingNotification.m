...
-(id)initWithTitle:(NSString *)_title image:(UIImage *)_image{
    ...
    self.titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(40, 40, 40, 40)];
    self.titleLabel.text = _title;
    [self addSubview:self.titleLabel];
    
    self.img = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 40, 40)];
    self.img.image=_image;
    [self addSubview:self.img];
    self.type=FloatingNotificationTypeTextIcon;
    return self;
}
-(id)initWithFrame:(CGRect )frame title:(NSString *)_title image:(UIImage *)_image{
    self=[self init];
    self.title=_title;
    ...
    self.titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(0,frame.size.height-40, frame.size.width, 40)];
    self.titleLabel.text = _title;
...
-(CGSize)getDefaultSizeInScale:(float)scaleFactor{
    if(self.type==FloatingNotificationTypeNone){
        return CGSizeMake(0*scaleFactor, 0*scaleFactor);
    }else if(self.type==FloatingNotificationTypeTextOnly){
        return CGSizeMake([self getMaxLength]*scaleFactor,30*kFloatingNotificationBoundsSizeFactor*scaleFactor);
    }else {
...
//        self.img.frame = self.frame;
//        self.titleLabel.frame = CGRectMake(0, self.frame.size.height-40, self.frame.size.width, self.frame.size.height);
